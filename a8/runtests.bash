...
# WLP4 programs and not .wlp4i files. Each test "test.wlp4" should have a
...
# Change if you're using Racket, want to run with Valgrind, etc.
MY_COMPILER="./wlp4gen"
...
# Names of the print and alloc libraries - probably don't change these?
PRINT="print.merl"
ALLOC="alloc.merl"
...
function detect_mips() {
  ...
  wlp4scan < "$PROGRAM" > "$PROGRAM.scanned"
...
function runtest() {
  ...
  if wlp4c < "$PROGRAM" > "$PROGRAM.$EXPECTED_MARKER.mips"; then
    ...
    # MY_COMPILER is unquoted because otherwise running with Valgrind doesn't work (?)
    cd ..
    (cat "$1" | wlp4scan | wlp4parse | $MY_COMPILER) > "$TEST_DIR/$PROGRAM.$USER_MARKER.asm"
    cd "$TEST_DIR"
    # Now run the assembler...
    cs241.linkasm < "$PROGRAM.$USER_MARKER.asm" > "$PROGRAM.$USER_MARKER.merl"
    # Next is the linker.
    # Note PRINT and ALLOC will be empty if the user has not downloaded these libraries.
    # They're not quoted because passing an empty string to cs241.linker will cause issues.
    # By leaving them unquoted, it's as if the arguments don't exist at all.
    # cs241.linker will just produce a warning if the program requires these libraries.
